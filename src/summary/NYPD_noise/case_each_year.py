...
if __name__ == "__main__":
	...
	NYPD_year_type = line.filter(lambda x: (check_datetime(x[1]),x[3],x[5]))\
	                    .map(lambda x: (str(get_year(x[1])), x[3],x[5]))\
	                    .filter(lambda x : (x[1] == "NYPD"))\
	                    .map(lambda x: ((x[0], x[1], x[2]),1))\
	                    .reduceByKey(add)\
	                    .sortByKey()\
	                    .map(lambda x: (str(x[0][1])+ ',' + str(x[0][0])+','+ str(x[0][2])+','+str(x[1])))\
	                    .saveAsTextFile("NYPD_year_type.out")
