...
if __name__ == "__main__":
	...
	year_complaint_type = line.map(lambda x: (x[1].encode('utf-8'), x[5].encode('utf-8')))\
	                        .filter(lambda x : (check_datetime(x[0]),x[1]))\
	                        .map(lambda x: (str(get_year(x[0])),x[1]))\
	                        .map(lambda x: ((x[0],x[1]),1))\
	                        .reduceByKey(add)\
	                        .sortByKey()\
	                        .map(lambda x: (str(x[0][0])+ ',' + str(x[0][1])+','+str(x[1])))\
	                        .saveAsTextFile("year_complaint_type.out")